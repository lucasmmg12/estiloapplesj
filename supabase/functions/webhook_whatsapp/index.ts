...
serve(async (req) => {
  ...
  try {
    ...
    let telefono = payload.telefono || payload.from || payload.phone || '';
    ...
    if (!telefono && payload.user) {
      telefono = payload.user.phone || payload.user.id || '';
    }

    // Si el nombre viene vacío, usar el teléfono o un valor por defecto
    if (!nombre) {
      nombre = payload.user?.name || telefono || 'Cliente';
    }
    ...
    // Asegurar que tengamos teléfono
    if (!telefono) {
    ...
    const prompt = `Analiza la siguiente conversación de WhatsApp y responde SOLO con un objeto JSON válido (sin bloques de código markdown, sin explicaciones adicionales).
